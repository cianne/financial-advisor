...
// Background Images
function mb_get_background_images() {
	$backgrounds = glob( CHILD_DIR . '/images/background/{*.jpg,*.gif,*.png}', GLOB_BRACE );
	$result = array();

	foreach ( $backgrounds as $background ) {
		$basename = basename( $background );
		$path = MB_THEME_IMAGES . 'background/';
		$image = $path . $basename;
		$result[] = array(
			'value' => $image,
			'label' => $basename,
